...
model.folder.out <- file.path(out_dir, "forest/fore_mu_bgc/2/")
#model.folder.out <- file.path(out_dir, "forest/fore_mu_bgc/SBSmc2/_uc/predicted")

#acost_file <- "AccumCostSurface.tif
acost_file <- "accum_cost_mask.tif"
...
if(file.exists(file.path(out_path, "acost.tif"))){
  ...
  accum_cost <- raster(file.path(stage1_dir, acost_file)) 
...
# Read in masked exclusion layer 

#zones = "IDFdm1"
zones = "MSdm1"
cost_name <- paste0(zones, "_", acost_file)
cost_name <- gsub("mask.tif", "masked.tif", cost_name)
  
bgc_filter <- st_read(file.path(shapes_path, "bec.gpkg")) %>% 
    dplyr::filter(MAP_LABEL == zones)

accum_cost_masked <- raster(file.path(stage1_dir, cost_name)) 

bc3005 = "+init=epsg:3005"
accum_cost_masked = projectRaster(accum_cost_masked, crs = bc3005, 
                                 method = "bilinear"#"ngb"
                                 )

#accum_cost_masked5 <- disaggregate(accum_cost_masked, 5)


# # use stars: not working correctly yet 
# accum_cost_masked <- read_stars(file.path(stage1_dir, cost_name))
# st_crs (accum_cost_masked) <- 3005
# 
# # 25m resolution - calculate the change to make to 5m resolution 
# #xdim = dim(accum_cost_masked)[1]*5
# #ydim = dim(accum_cost_masked)[2]*5
# 
# rasterio = list(nXSize = 25, nYSize = 25, 
#                 nBufXSize = 125, nBufYSize = 125,
#                 resample = "nearest_neighbour", 
#                 bands = 1)
# 
# x = read_stars(file.path(stage1_dir, cost_name), RasterIO = rasterio)
# st_crs (x) <- 3005


mask_poly <- st_read(file.path(stage1_dir, paste0(zones, "_mask_poly.gpkg"))) %>%
    st_transform(3005) 
...
model.folder.out <- file.path(out_dir, "forest/fore_mu_bgc/2/")
model.folder.out = file.path(model.folder.out, zones)

list.files(model.folder.out)

covs <- list.files(file.path(model.folder.out, "predicted"), pattern = ".tif", full.names = TRUE)

pred_covs <- stack(covs[grep("^prob.", basename(covs))])
pred_covs <- projectRaster(pred_covs, crs = bc3005, 
                                 method = "bilinear"#"ngb"
                                 )
xxxx <- pred_covs
...
entropy <- raster(list.files(model.folder.out, pattern = "entropy.tif$", full.names = TRUE))

# reproject if needed: 
entropy <- projectRaster(entropy, crs = bc3005, 
                                 method = "bilinear" )#"ngb"
                                 
# mask the entropy layer by bgc then create a histogram 
r <- raster::mask(entropy, as(bgc_filter,'Spatial'))
entropy = r

hist(entropy)
#entropy[entropy == 0] <- NA

## filter entropy to the most uncertain level 
#entropy <- raster(covs[grep("^entropy.", basename(covs))])
#entropy[entropy <= 0.75] <- NA # deception cut off

# boundary entropy is normal distribution (-0.)
#entropy[entropy <= -0.75] <- NA # IDFdm1
entropy[entropy <= 5] <- NA # MSdm1
hist(entropy)
#entropy05 <- entropy[entropy<= -0.5] <- NA  # test other sampling options 
#entropy075 <- entropy[entropy<= -0.75] <- NA

writeRaster(entropy, file.path(stage2_dir, "MSdm1_entropy.tif"), format = "GTiff", overwrite = TRUE)
...
writeRaster(entrop_5000 , file.path(stage2_dir, "MSDm1__entropy_sieved.tif"), format = "GTiff", overwrite = TRUE)

entropy_5000 <- raster(list.files(stage2_dir, "MSDm1_entropy_sieved.tif", full.names = TRUE))

entropy <- entropy_5000
entropy <- crop(entropy, pred_covs)

# prep cost layer: 
cost <- disaggregate(accum_cost_masked, 5)
cost <- crop(cost, pred_covs)
pred_covs <- crop(pred_covs, cost)
...
writeRaster(anc_layers_masked, file.path(stage2_dir, "MSDm1_anc_layers_masked.tif"), format = "GTiff", overwrite = TRUE)
...
s <- sampleRegular(anc_layers_masked , 
                   size = 50000, 
                   sp = TRUE) # sample raster
...
#   
  ...
  #   i <- 1
    if(nrow(sample_points) < slice_size*i){
      ...
      sample_points <- rbind(sample_points, clhs_sampled)
    }
  }
...
# read in points: 
sample_points <- st_read(file.path(stage2_dir, "entropy", "IDFdm1", "entropy_pts.shp")
)
...
## Extract the 1st point
      ...
      cLHS_tris <- rbind(cLHS_tris, triSet)
...
st_write(all_points, file.path(stage2_dir, "MSdm1_all_pointsV2.gpkg"),delete_layer = TRUE)

st_write(cLHS_points, file.path(stage2_dir, "MSdm1_clhs_pointsV2.gpkg"),delete_layer = TRUE)
