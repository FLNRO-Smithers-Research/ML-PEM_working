...
# read in the aoi
aoi <- st_read(file.path(shape_dir, "aoi_snapped_qa.gpkg")) %>%
  mutate(id = 1)

# create a set of template rasters based on various cell resolution

res <- c(5) # set to 1m, 5m and 25m 
...
#  writeRaster(dem_aoi_prj, file.path(dem_dir, "lidar","lasmetrics_5m", folders, fname), format = "GTiff", overwrite = TRUE)
    writeRaster(dem_aoi_prj, file.path(dem_dir, "lidar","lasmetrics_5m", fname), format = "GTiff", overwrite = TRUE)
...
```{r neighbourhood interpolation}
###Will inserted
library(terra)
require(tictoc)
#terra 1.5.6
#v <- vect(system.file("ex/lux.shp", package="terra"))
r <- rast(file.path(r_dir, "cov_gap_mosaic.tif"))
tic()
f1 <- terra::focal(r, 5, "median", na.policy="omit", na.rm=TRUE)
toc()
writeRaster(f1, file.path (r_dir, "cov_gap_mosaic_5xnbr.tif"))
...
Generate coarser scale covariates: 

```{r}
#downscale the p90 for 25m and class for the remote sensed data
AOI <- "DateCreek"
res <- "25m" 
AOI_dir <- file.path(".", paste0(AOI,"_AOI"))
cov_dir <- file.path(AOI_dir, "1_map_inputs", "covariates")
...
p90 <- list.files(file.path(cov_dir, "5m"), full.names = T, pattern = "p90_mosaic_rproj.tif$") 
#all <- all[14:48]
...
rc1 <- classify(p90, rclmat, include.lowest=TRUE)
w_out <- writeRaster(rc1, file.path(cov_dir, "5m", "p90_classes_raw.tif"), overwrite = TRUE)


#downscale to 25m

rtemplate = rast(file.path(cov_dir, "25m", "template.tif"))


    template_res <- res(rtemplate)
    template_ext <- extent(rtemplate)
    r <- rc1
    dres <- res(r)[1]
    px <- raster::res(r)[1]
    r25  <- terra::aggregate(r, fact = 5, method = "mean") 
    
    #rr <- c(r25, rtemplate)
    
    w_out <- writeRaster(r25, file.path(cov_dir, res, "p90_classes_raw.tif"), overwrite = TRUE)

    , px/template_res)  ## This will through an error if not an integer
...
w_out <- writeRaster(rc1, file.path(cov_dir, res, "p90_classes_raw.tif"), overwrite = TRUE)
...
# Generate classes for SAVI fo remote sensed 

```{r}

#generate classes for SAVI data set 
AOI <- "DateCreek"
AOI_dir <- file.path(".", paste0(AOI,"_AOI"))
cov_dir <- file.path(AOI_dir, "1_map_inputs", "covariates")
...
savi <- list.files(file.path(cov_dir, "25m"), full.names = T, pattern = "sen_savi.tif$") 
savi <- rast(savi)
# all values >= 0 and <= 0.25 become 1, etc.
m <- c(-5, 0.2, 1,
       0.2, 0.75, 2,
       ...
       1.13, 5, 4)
rclmat <- matrix(m, ncol=3, byrow=TRUE)
rc1 <- classify(savi, rclmat, include.lowest=TRUE)
w_out <- writeRaster(rc1, file.path(cov_dir, "25m", "sen_savi_6Class.tif"), overwrite = TRUE)
