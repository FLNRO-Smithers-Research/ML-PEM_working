...
nearest_town = "Smithers"   # SW2


# if using trim vs Lidar derived 25m 
if(use_trim == T){
...
roads <- st_read(file.path(shapes_path, "road_surroundings.gpkg")) %>%  
    st_zm() #%>% 

roads <- st_read(file.path(shapes_path, "roads_vetted_detail.gpkg")) %>%  
    st_zm()# %>%
  # mutate(ROAD_SURFA = ROAD_SURFACE) %>%
  # mutate(ROAD_NAM_7 = ROAD_NAME_FULL)
...
writeRaster(rdsRast,  file.path(out_path,"input_raster", "test.tif"), format = "GTiff", overwrite = TRUE)
...
We can check the distribution of costs per landscape variability (classes) as generated 
for the basis of landscape distrubution. This will review how much sampling space is available for each of the bgc units.  


```{r check BGC cost distribution}
...
# match to original extent of acost
...
for (b in boi) {
 b = boi[1]
  ...
  writeRaster(boi_mask, file.path(out_path, "input_raster", paste0(b,"_exclude_mask.tif")), overwrite = TRUE)  
  
# write out polygon version for faster processing 
   
  mask_poly_boi <- 1 + (boi_mask *0) 
  mask_poly_boi <- st_as_stars( mask_poly_boi, crs = st_crs(3005))
...
#boi <- c("ESSFwh 1","ESSFwh 2","ICH wk 1" ) # 15
...
for (b in boi) {
  ...
  paired_distance <- 100
  num_samples <- 5   # total no of samples wanted
...
clhs_outpath <- file.path(out_path, "clhs_sample_plans", "final")

boi <- "ESSFwc 4"
clhs_set <- "clhs_pts1_ESSFwc 4.gpkg"
...
#boi <- c("ICH mk 4", "IDF xx 2", "ESSFwm 1","IDF dm 2","ESSFdk 1", "ICH dm","ESSFwm 4", "MS  dw") # 
boi <- c("ICH mw 2", "ICH wk 1", "ESSFwh 1", "ESSFwh 2", "ESSFwm 2","ESSFwc 4") 


for (ii in 1:length(boi)) {
# ii = 1
  b <- boi[ii]
  points <- st_read(file.path(transect_out_path, outname), layer = paste0(b,"_points")) 
  points.out <- points %>%
...
outname = "s1_sampling_NE.gpkg"


points <- st_layers(file.path(transect_out_path, outname)) 
boi <- points$name[grep("_clhs",points$name)]
boi <- gsub("_points_clhs","",boi)
