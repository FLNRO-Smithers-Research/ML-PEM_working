---
title: "Stage2 Sampling_Ensemble"
...
## Stage 2 sampling _ ensemble method

This methodology outlines option 2 for the stage 2 sample: using the model disagreement to sample in areas where mutiple learners disagree about the model types. 

This relies on input from 05c_PEM_iAA_Ensemble_ModelAssessment.R script, which runs an ensemble machine learning model (ranger,  xgboost, )
...
bgc = "MSdm1" 
#model.folder.out <- file.path(out_dir, "forest/fore_mu_bgc_S2/SBSmc2/predicted")
model.folder.out <- file.path(out_dir, paste0("forest/fore_mu_bgc/3/",bgc,"/predicted"))
...
accum_cost <- raster(file.path(stage1_dir, paste0(bgc,"_accum_cost_masked.tif")))
 #"AccumCostSurface.tif")) 
...
st_write(mask_poly, file.path(paste0(stage2_dir,"/02_ensemble"), "mask_poly.gpkg"),delete_layer = TRUE)
...
#covs <- "D:/PEM_DATA/BEC_DevExchange_Work/Deception_AOI/3_maps_analysis/models/forest/fore_mu_bgc_S2/SBSmc2/predicted/submodel/mosaic/"

#covs <- list.files(covs, pattern = ".tif", full.names = TRUE)
#covs <- covs[-3]
#pred_covs <- stack(covs)



## read in the model agreement layer 

#covs <- "D:/PEM_DATA/BEC_DevExchange_Work/Deception_AOI/3_maps_analysis/models/forest/fore_mu_bgc_S2/SBSmc2/predicted/submodel/mosaic/"

covs <- file.path(model.folder.out)

covs <- list.files(covs, pattern = "proportion.tif", full.names = TRUE)
agree = covs


# anything that is 0.75 and greater is ignores (ie 3 or more models agree). ie tagret areas with models are confused: 

#https://rdrr.io/github/dutri001/bfastSpatial/
#entropy <- raster(covs[grep("^submod_mosaic.", basename(covs))])
entropy <- raster(agree)
...
# testing various sieving options:

entrop_5000 <- areaSieve(entropy, thresh = 5000, directions = 8, verbose = T, cores = detectCores()-2)

writeRaster(entrop_5000 , file.path(paste0(stage2_dir,"/ensemble"), "MSdm1_agree_5000.tif"), format = "GTiff", overwrite = TRUE)
...
entrop_10000 <- areaSieve(entropy, thresh = 10000, directions = 8, verbose = T, cores = detectCores()-2)

#writeRaster(entrop_10000 , file.path(paste0(stage2_dir,"/ensemble"), "IDFdm1_agree_100001.tif"), format = "GTiff", overwrite = TRUE)

entrop_25000 <- areaSieve(entropy, thresh = 25000, directions = 8, verbose = T, cores = detectCores()-2)

#writeRaster(entrop_25000 , file.path(paste0(stage2_dir,"/ensemble"), "IDFdm1_agree_25000.tif"), format = "GTiff", overwrite = TRUE)



#writeRaster(entrop_5000 , file.path(stage2_dir, "m_agree_5000.tif"), format = "GTiff", overwrite = TRUE)

entropy <- entrop_5000
#entropy <- crop(entropy, pred_covs)

# prep cost layer: 
cost <- disaggregate(accum_cost, 5)
#cost <- crop(cost, entropy)
...
anc_layers_masked <- mask(cost, entropy) %>%
  mask(cost)

s1 <- sampleRandom(anc_layers_masked , size = 50, sp = TRUE) # sample raster
s2 <- sampleRandom(anc_layers_masked , size = 50, sp = TRUE) # sample raster


sample_points1 <- st_as_sf(s1)
sample_points2 <- st_as_sf(s2)

#st_write(sample_points1, file.path(paste0(stage2_dir,"/ensemble"), "IDFdm1_random_pt1.gpkg"), delete_layer = TRUE)

#st_write(sample_points2, file.path(paste0(stage2_dir,"/ensemble"), "IDFdm1_random_pt2.gpkg"),
#         delete_layer = TRUE)

#st_write(sample_points, file.path(paste0(stage2_dir,"/02_ensemble"), #"test_pts.gpkg"),delete_layer = TRUE)
...
# clhs to remove
to_remove <- c("SBSmc2_2.1_6","SBSmc2_4.5_15", "SBSmc2_4.4_14", "SBSmc2_1.4_4")

cLHS_points <- cLHS_points %>%
  dplyr::filter(!id %in% to_remove)
...
## Extract the 1st point
      ...
      # select best based on min model_agreement  value
      cLHS_tri  <- cLHS_tris  %>%
         group_by(id) %>%
          slice(which.min(Entropy_Sum)) %>%
          ungroup() 
...
for(i in 1:nrow(sample_points_clhs)){
 #i = 1
  ...
  rotated_points <- pairedPoint(pnt, centroid_distance, rotation_angles) %>%  # 8 points 400 m away
    ...
    dplyr::filter(!is.na(AccumCostSurface)) 
...
sample_points_rotations <- sample_points_rotations %>% 
  dplyr::select(-c(AccumCostSurface))#layer))
...
all_points <- bind_rows(sample_points_clhs, sample_points_rotations) %>%
 ...
 toremove <- c("SBSmc2_2.5_10_SW", "SBSmc2_7.3_23_N", "SBSmc2_7.3_23_E",
               "SBSmc2_7.3_23_SE", "SBSmc2_7.3_23_N", "SBSmc2_7.1_21_S", 
              "SBSmc2_6.3_18_W", "SBSmc2_4.3_18_NW", "SBSmc2_2.3_8_NE",
              "SBSmc2_2.3_8_E", "SBSmc2_2.3_8_N") 
...
all_points_out <- all_points %>%
  mutate(id = paste0(id, "_", "e")) 

cLHS_points <- cLHS_points %>%
  mutate(id = paste0(id, "_", "e")) 

st_write(all_points_out, file.path(paste0(stage2_dir, "/02_ensemble"), "all_points.gpkg"),delete_layer = TRUE)

st_write(cLHS_points, file.path(paste0(stage2_dir, "/02_ensemble"), "clhs_points.gpkg"),delete_layer = TRUE)
...
#cLHS_points_edit <- cLHS_points %>%
#  dplyr::filter(!id %in% to_remove)

#"SBSmc2_3.1_6","SBSmc2_3.4_9", "SBSmc2_3.5_10", #"SBSmc2_4.2_12", "SBSmc2_5.1_16", "SBSmc2_5.3_18")
...
all_triangles_ent <- all_triangles_ent %>%
   mutate(id = paste0(id, "_e")) 

sample_points_slices <- sample_points_rotations %>% 
  mutate(id = paste0(id, "_", rotation, "_e")) %>% 
  dplyr::select(slice_site, slice_num, id) %>% 
  st_drop_geometry()

all_triangles_ent <- left_join(all_triangles_ent, sample_points_slices )


#st_write(all_triangles_ent , file.path(paste0(stage2_dir, "/02_ensemble"), #"all_tri_edit.gpkg"),delete_layer = TRUE)

toremove <- c("SBSmc2_2.5_10_SW_e", "SBSmc2_7.3_23_N_e", "SBSmc2_7.3_23_E_e",
               "SBSmc2_7.3_23_SE_e", "SBSmc2_7.3_23_N_e", "SBSmc2_7.1_21_S_e", 
              "SBSmc2_6.3_18_W_e", "SBSmc2_4.3_18_NW_e", "SBSmc2_2.3_8_NE_e",
              "SBSmc2_2.3_8_E_e", "SBSmc2_2.3_8_N_e") 

all_triangles_ent1 <- all_triangles_ent %>%
  filter(!id %in% toremove)

#st_write(all_triangles_ent1 , file.path(paste0(stage2_dir, "/02_ensemble"), "all_tri_edit1.gpkg"),delete_layer = TRUE)

#"SBSmc2_4.3_13_N", "SBSmc2_4.3_13_NE", "SBSmc2_4.3_13_E"

all_triangles_ent <- all_triangles_ent1

# select the triangle with the max entropy, add clhs tris and write out
 paired_triangles <- all_triangles_ent %>% 
   group_by(slice_site, slice_num) %>%
   slice(which.min(Entropy_Sum)) %>% 
   ungroup() %>%
...
# 
 cLHS_tri <- cLHS_tri %>% 
   mutate(id = paste0(id, "_","CLHS_e")) %>% 
   dplyr::select(id, Entropy_Sum)
 
 sample_tri <- bind_rows(cLHS_tri, paired_triangles)
 
 st_write(sample_tri , file.path(paste0(stage2_dir, "/02_ensemble"), "sample_tri.gpkg"),delete_layer = TRUE)

 
 # write out a tracking sheet: 
 tracking_sheet <- sample_tri %>%
   st_drop_geometry()
 
 write.csv(tracking_sheet, file.path(paste0(stage2_dir, "/02_ensemble"), "tracking_sheet.csv"))
...
st_write(all_tri , file.path(paste0(stage2_dir, "/02_ensemble"), "all_tri.gpkg"), delete_layer = TRUE)


####write buffer#########################
tri_buff <- st_buffer(all_tri, dist = 10)

st_write(tri_buff, file.path(paste0(stage2_dir, "/02_ensemble"), "all_tri_buf.gpkg"), delete_layer = TRUE)
