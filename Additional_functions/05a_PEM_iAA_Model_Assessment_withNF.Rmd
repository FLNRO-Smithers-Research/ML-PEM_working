...
tpts_nf <- tpts %>%
  ...
  #left_join(map.key.type, by = c("mapunit2" = "BaseMapUnit")) %>%
  #mutate(mapunit2_type = Type) %>%
  #dplyr::select(-Type) %>%
  #dplyr::select(mapunit1_type, mapunit2_type,  fnf,  mapunit1, mapunit2 ) %>%
  #distinct()
...
nfor.sum <- mpts %>%
  dplyr::select(target, forest_nonforest) %>%
  dplyr::group_by(target, forest_nonforest)%>%
  dplyr::summarise(sum = length(forest_nonforest))%>%
  pivot_wider(names_from = forest_nonforest, values_from = sum)
...
# if (str_detect(mname, 'bgc')) {
  ...
  bgc_pts_subzone <- lapply(zones, function (i){
      ...
      #i <- zones[1]
      #ii <- zones[str_detect(zones, paste0("\\b", i, "\\b"))]
      ...
      pts_tid <- unique(pts_subzone$tid)
      

      if(nrow(pts_subzone) == 0){ pts_subzone = NULL} else {
        
      nonfor <- c("nfor", "non_veg") # fix XVT for SBSmc2 also
      
      pts_subzone_nf <- mpts %>%
        filter(target %in% nonfor) %>%
        filter(tid %in% pts_tid)
      
      pts_subzone <- bind_rows(pts_subzone, pts_subzone_nf) %>%
        droplevels()
  ...
  #model_bgc <- lapply(names(bgc_pts_subzone), function(xx) {
    ...
    # ESSFmc 
    ds_ratio = 15 #40  #15   #40  #(0 - 100, Null = 100)
    sm_ratio = 0.2 #0.6 #0.2 #0.7 #0 - 1, 1 = complete smote
