---
title: "Build Forest/NonForet Deception Test Case"
...
```{r load data, echo = FALSE, include = TRUE}
 trDat <-  fread("D:/GitHub/PEM_Methods_DevX/Deception_AOI/1_map_inputs/trainingData/S1_AllCovariates.csv")
trDat <- trDat %>% dplyr::rename(target = mapunit1, target2 = mapunit2)# %>% filter(bgc == 2)
table(trDat[, fnf])
LiDAR <- fread("../Deception_AOI/_MapUnitLegend/newLiDARcovs.csv") %>% dplyr::rename(Lidar.covs = 1)
testunits = c("forest", "non_forest")
trDat <- trDat  %>% filter(fnf %in% testunits )%>% dplyr::select(fnf,bgc, LiDAR$Lidar.covs)# %>% 
  #dplyr::select(fnf, target, target2, tid, cov_gap_mosaic, vc1_mosaic)
# trDat <- trDat %>% dplyr::select(target, target2, tid,  362:402)
# LiDAR <- colnames(trDat) %>% data.frame#cov_gap_mosaic, p60_mosaic, kur_mosaic, p85_mosaic)
# fwrite(LiDAR, "../Deception_AOI/_MapUnitLegend/newLiDARcovs.csv")

MU.for <- map.key %>% dplyr::select(MapUnit, Type)  %>% distinct()#%>% filter(Type == "For")
MU.for <- MU.for$MapUnit
##tpts <- tpts %>% filter(target %in% MU.for)
MU_count2 <- trDat %>% dplyr::count(fnf)
  mpts <- trDat
...
ggplot(trDat, aes(fnf)) +
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 90))

```
```{r groups data by map legend, echo = FALSE, include = TRUE}
# group.select <- "Type2"
# MU.grp <- map.key %>% dplyr::select(BaseMapUnit, group.select)  %>% distinct() %>% dplyr::rename("target" = 1, "newgroup" = 2) #%>% filter(Type == "For")
# trDats <- trDat[
#     MU.grp,
#     on = c(target = target),
#     target := newgroup
...
#trDats <- trDats[complete.cases(trDats[ , 13:length(trDats)]),]
#trDats <- trDats %>% filter(!is.na(target)) %>% filter(!target == "")

# MU.grps <- unique(MU.grp$newgroup)
# ##tpts <- tpts %>% filter(target %in% MU.for)
# MU_count2 <- trDats %>% dplyr::count(target)
#   mpts <- trDats
# 
#   ## filter for pure calls only
#   mpts$target2 <- mpts$target2 %>% na_if("")
...
#BGC_all <- trDats %>% dplyr::select(-fid, -comments, -data_type, -target2, -all_mosaic)
# BGC_all <- trDats %>% dplyr::select(-target2) %>% 
#   as.data.frame %>% replace(is.na(.), 0) %>% mutate_if(is.integer, as.numeric)


#, -x, -y, -bgc_cat, -geomdplyr::select(-fnf, -transect_id,  -slice, -data_type, -target2, -slice, -transition, -observer, -comments) 

#BGC_all$target <- as.factor(BGC_all$target) 
#BGC_all <-  BGC_all %>% purrr::set_names(~ str_to_lower(.) %>% str_replace_all("_", ""))
                  
#BGC_all <- BGC_all %>% dplyr::select(target, tid, covgapmosaic, stdmosaic, vc0mosaic, vc1mosaic, p90)
# BGC_all <- trDat %>% dplyr::select(-transect_id, -slice, -transition, -comments, -data_type, -observer, -x, -y, -geom, -bgc_cat) %>% 
# #BGC_all <- trDat %>% 
#   as.data.frame %>% replace(is.na(.), 0) %>% mutate_if(is.integer, as.numeric) %>% dplyr::select(-target, -target2) %>% 
# dplyr::rename(target = fnf)
...
balance_recipe <-  recipe(target ~ ., data =  BGC_all) %>%
    update_role(tid, slice, new_role = "id variable") %>% 
    step_corr(all_numeric(), threshold = .8) %>%        # remove correlated covariates
    #step_dummy(all_nominal(),-all_outcomes()) %>%    
    # step_zv(all_numeric()) %>% # remove values with no variance
     #step_downsample(target, under_ratio = 10) %>%
  #step_smote(target, over_ratio = 1, neighbors = 10) %>% 
  #step_dummy(bgc) %>%      
  step_interact(terms = ~ bgc:cov_gap_mosaic) %>% 
    prep()

BGC_all2 <- balance_recipe  %>% juice()#BGC_train2 <- PEM_recipe %>% prep() %>% juice()
ggplot(BGC_all2 , aes(target)) +
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 90))+
  ggtitle("Up/downsampled set")

BGC.split <- initial_split(BGC_all2, prop = .8, strata = slice)

BGC_train <- training(BGC.split)

BGC_test <- testing(BGC.split)

```

```{r}
require(DataExplorer)
plot_correlation(BGC_all2)
plot_boxplot(BGC_all2, by = "target")
plot_prcomp(BGC_all2, maxcat = 5L)
```

## build  train model and predict test data

```{r fit non-cv model , error=TRUE}
randf_spec <- rand_forest(mtry = 4, min_n = 10, trees = 100) %>%
  set_mode("classification") %>%
  set_engine("ranger", importance = "impurity") 

balance_recipe <-  recipe(target ~ ., data =  BGC_train) %>%
    update_role(tid, slice, new_role = "id variable") %>% 
    #step_corr(all_numeric(), threshold = .7) %>%        # remove correlated covariates
    #step_dummy(all_nominal(),-all_outcomes()) %>%    
    # step_zv(all_numeric()) %>% # remove values with no variance
      step_downsample(target, under_ratio = 10) %>%
   step_smote(target, over_ratio = 1, neighbors = 10) %>% 
      #step_interact(terms = ~ vc1_mosaic:cov_gap_mosaic) %>% 
     prep()

#or "permutations
#BGC_all$target <- factor(BGC_all$target)
# BGC_all <-  BGC_all %>% purrr::set_names(~ str_to_lower(.) %>%
#                   str_replace_all("_", ""))
# balance_recipe <-  recipe(target ~ ., data =  BGC_train2) %>%
# prep()

data_baked <- bake(balance_recipe, new_data = BGC_test)
    # step_zv(all_numeric()) %>% # remove values with no variance
...
print(PEM_rf1)
  ...
  extract_fit_parsnip(PEM_rf1) %>% 
  vip(num_features = 20)

```


```{r predict with model , error=TRUE}
predictions <- PEM_rf1 %>% predict(new_data = BGC_test) %>% 
  bind_cols(data_baked %>% dplyr::select(target)) %>%  accuracy(truth = target, .pred_class)
predictions
PEM_rf1 %>% predict(new_data = data_baked) %>% 
  bind_cols(data_baked %>% dplyr::select(target))%>% conf_mat(.pred_class, target) %>% 
  pluck(1) %>%
  as_tibble() %>%
...
BGC_all <- BGC_all %>% dplyr::select(-tid)
final_recipe <-  recipe(target ~ ., data =  BGC_all) %>%
    #update_role(tid, new_role = "id variable") %>% 
    #step_corr(all_numeric()) %>%        # remove correlated covariates
    #step_dummy(all_nominal(),-all_outcomes()) %>%    
    # step_zv(all_numeric()) %>% # remove values with no variance
  #    step_downsample(target, under_ratio = 50) %>%
...
model_final <- fit(best_workflow, data = BGC_all)

final_fit <- extract_fit_parsnip(model_final)# %>%pull(.predictions)
variables <- final_fit %>% vip(num_feature = 4)
variables
oob  <- round(model_final$fit$fit$fit$prediction.error, 3)
oob
saveRDS(model_final , "../Deception_AOI/3_maps_analysis/models/ForNfor_tidy_model.rds")

```

```{r predict with model , error=TRUE}
######### Predict Test
test_target <-as.data.frame(BGC_test$target) %>% dplyr::rename(target = 1)
