...
outDir <- file.path(out_dir, "forest_non_forest", "for_nfor_all", mrep)

# OR 
# manual set up model parameters

#map_res <- 5
#mname = "fore_mu_bgc" # or
#mname = "fore_mu_all"
# 
# #modeltype = "tidy_model.rds" #for tidy model 
...
#rFNonFor <- readRDS("./Deception_AOI/3_maps_analysis/models/ForNfor_tidy_model.rds")
#xx <- rFNonFor$fit$fit$fit$variable.importance %>% as.data.frame %>% row.names()


```

```{r kmeans clusters of }
...
predict_map_tidy(
        #model = "./Deception_AOI/3_maps_analysis/models/ForNfor_tidy_model.rds",
...
#       # testing map production (one variant)
#       
#       #bec_aoi = bec_shp %>% filter(MAP_LABEL == bgc)
#       
#       predict_map_tidy(
#         model = file.path(outDir, modeltype),
#         cov = rast_list,
...
# aspect <- raster(rast_list[1])
# bgc <- raster(rast_list[2])
# 
# crs(bgc) = crs(aspect) 
# # read in as gdal and saga (or regenerate from scratch).
# 
# writeRaster(bgc, "D:/PEM_DATA/BEC_DevExchange_Work/Deception_AOI/1_map_inputs/covariates/5m/bgc1.tif")
...
bec_shp <- st_read(file.path(shapes_dir, "bec.gpkg"), quiet = TRUE) %>%
  mutate(BGC_LABEL = gsub(" ","",MAP_LABEL))
...
map_dir = "D:/PEM_DATA/BEC_DevExchange_Work/Deception_AOI/3_maps_analysis/models/forest/fore_mu_bgc/89"

folders <- as.list(c("ESSFmc", "ESSFmcw", "SBSmc2")) 
#folders <- as.list(c("ESSFmc","SBSmc2")) 
#f = "SBSmc2"

# set up a key for the combined map 
...
combo_map <- lapply(folders, function(f){
  ...
  # need to filter to only predict over bgc
  ...
  if (isTRUE(unique(subkey$X == subkey$map.response))) {
   ...
   print("updating key")
    ...
    reclm <-  as.matrix(m, ncol=3)
    rtemp <- reclassify(rtemp, reclm, include.lowest=TRUE)
  
   # sort(unique(values(rtemp)))
  }
...
# tidy key 
rkey <- rkey %>% 
  dplyr::select(x, model, map.response)
